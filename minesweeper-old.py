...
# Author: Steven Chen
# Description: Enter text commands into the terminal to play Minesweeper on a 10x10 grid!

from border_cell import *
from cells import *
...
class Minesweeper:
    ...
    def game_over(self):
        """sets the game as over and displays a "Game Over" message to the player"""
    ...
    def user_action(self, act_row, act_col, act_opt):
    ...
    def print_board(self):
        """prints the game board following standard rules"""
    ...
    def get_input(self):
        ...
        try:
            ...
            print("You entered a row or column number that is out of bounds")
            self.get_input()
        ...
        u_o = user_input[2]
        try:
            self.user_action(int(u_r), int(u_c), u_o)
        except InputFlagError:
    ...
    def check_input_list(self, input_list):
        ...
        if len(input_list) == 1:
            try:
                self.confirm_cheat(input_list[0])
            except InputCheatError:
    ...
    def confirm_cheat(self, cheat_string):
        """if the user only entered 1 string, check if it is a cheat code"""

        if cheat_string == "showall":
            print("")
            self.cheat_print_board()
            print("")
            self.get_input()
        elif cheat_string == "winpls":
            self.cheat_reveal()
        else:
            raise InputCheatError

    def cheat_print_board(self):
        """prints a board with all the mines and numbers (i.e. removes fog of war)"""
            ...
            for cheat_col in range(0, 12):
                ...
                if cheat_col == 11:
                    if cheat_cell.is_mine():
                        print("[ * ]")
                    # option used in debugging to check the hint numbers are displayed properly
    ...
    def cheat_reveal(self):
        """reveals every cell that does not contain a mine (this gives you a win)"""
